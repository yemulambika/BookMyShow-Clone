...
function MovieForm({ isModalOpen, setIsModalOpen, formType , selectedMovie , setSelectedMovie }) {
  const onFinish = async (values) => {
    if (formType == "add") {
      try {
        ...
        try {
        const respone = await updateMovie({...values , movieId:selectedMovie._id});
  ...
  return (
    <Modal open={isModalOpen} width={800} onCancel={handleCancel}>
      <Form   initialValues={selectedMovie} layout="vertical" style={{ width: "100%" }} onFinish={onFinish}>
        <Row
          gutter={{
            xs: 6,
            sm: 10,
